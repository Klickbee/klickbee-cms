...
function cloneContent(content: unknown): BuilderComponent[] {
	...
	// When header content is a single object, normalize to array
...
export function useHeaderEditor(pageId?: number) {
	...
	function withWorking<T>(
		...
		if (!pageId) return;
		const working = cloneContent(header?.content ?? []);
		const result = fn(working);
		setHeaderContent(working as unknown as BuilderComponent[]);
		return result;
	}
	...
	function containsNode(id: string): boolean {
		const content = cloneContent(header?.content ?? []);
